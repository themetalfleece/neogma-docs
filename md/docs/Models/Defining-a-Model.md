...
```js
    ...
    /** --> reverses the configuration of the relationship, so it's perfectly duplicated when definition the same relationship at the other model (here Orders) */
    Users.reverseRelationshipByAlias('Orders');  // --> { model: Users, direction: 'in', name: 'CREATES', properties: {...} }
